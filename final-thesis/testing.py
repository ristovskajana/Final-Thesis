...
def save_and_plot():
    ...
    pl.savefig(f"{counter}_score_plot.png")  # Save the plot as an image file
    ...
    pl.savefig(f"{counter}_training_plot.png")  # Save the plot as an image file

    pl.scatter(training_steps_per_episode, episode_scores)
    pl.xlabel("Training steps")
    pl.ylabel("Score")
    pl.title("Score/Training steps")
    pl.savefig(f"{counter}_score_training_plot.png")  # Save the plot as an image file


    # Save episode rewards as a JSON file
    with open(f"{counter}_episode_scores.json", "w") as f:
        json.dump(episode_scores, f)

    with open(f"{counter}_training_steps.json", "w") as f:
    ...
    while run:
        ...
        if not dead:
            if flappy.last_state == None:
                ...
                reward = 0
                flappy.update_q_table(flappy.last_state, flappy.last_action, reward, flappy.state, q_table)
            ...
            if pygame.sprite.groupcollide(bird_group, pipe_group, False, False) or flappy.rect.top < 0 :
                ...
                next_state = None
                reward = -1000
                flappy.update_q_table(flappy.last_state, flappy.action, -1000, next_state, q_table)
            ...
            if dead == False:
                ...
                flappy.update_q_table(flappy.last_state, flappy.action, reward, next_state, q_table)  # Update the Q-table using the flappy object
                    # print("For state: ", flappy.last_state, " and action performed: ", flappy.action, " new q value is: ", q_table[flappy.last_state][flappy.action])
...
# Function to automate testing for maximum score of 100
def automate_testing(folder_path):
    global counter, q_table_user

    data_files = os.listdir(folder_path)
    random.shuffle(data_files)

    num_test_files = min(200, len(data_files))
    test_files = data_files[:num_test_files]
    ...
    for file_name in test_files:
        ...
        # Reset the Q-table to its initial values for the next file testing
        q_table_user = np.zeros((num_discrete_states, num_actions), dtype=float)
        counter += 1
...
def main():
    global episode_scores, training_steps_per_episode
    initialize_pygame()
    # Perform testing and other tasks here
    data_folder_path = r"C:\Users\janar\Desktop\user_data\all_except_0"
    automate_testing(data_folder_path)
    ...
    episode_scores = []
    training_steps_per_episode = []
    # Perform any cleanup tasks before exiting the program
    cleanup_pygame()
